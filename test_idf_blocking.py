...
LEGAL = {'pvt', 'ltd', 'private', 'limited', 'inc', 'corp', 'corporation', 'llc', 'llp', 'co', 'company', 'services', 'solutions'}
...
def extract_blocking_keys(name, addr):
    ...
    for i, t in enumerate(toks[:4]):
        keys.append(f"w:{t}")
        # 3-char prefix for fuzzy matches
        if len(t) >= 4:
    ...
    addr_toks = [t for t in addr_clean.split() if len(t) >= 3 and t not in {'street', 'road', 'avenue', 'lane', 'drive', 'floor', 'suite', 'unit'}]
...
print("--> [3/4] Indexing ALL ~3 Million US Records of Source 2 with IDF Weights...")
t0 = time.time()
...
# Pre-compute IDF for each key: log(N / DF)
N = float(us_s2_count)
key_idf = {}
for k, id_list in idx_s2.items():
    df = len(id_list)
    # Filter out mega stop-keys that appear in > 10,000 documents
    if df > 10000:
        continue
    key_idf[k] = math.log((N + 1.0) / (df + 1.0))

print(f"Filtered keys with DF <= 10,000: {len(key_idf)} / {len(idx_s2)}")

# Query validation S1 entities with IDF scoring
print("--> [4/4] Querying validation entities with IDF-weighted candidate ranking...")
t0 = time.time()
found_s2 = 0
total_cands = 0
    ...
    for k in keys:
        if k in key_idf:
            ...
            for int_id in idx_s2[k]:
                cand_scores[int_id] += idf

    # Take top 35 candidates by total IDF score
...
q_time = time.time() - t0
print("\n=======================================================")
print(f"IDF-WEIGHTED US S2 BLOCKING EVALUATION:")
