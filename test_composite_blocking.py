...
def get_composite_keys(name, addr):
    ...
    keys = []
    # High-precision composite: name_word + num
    for nw in name_words[:3]:
        for nm in nums[:2]:
            keys.append(f"WN:{nw}_{nm}")

    # High-precision composite: name_word1 + name_word2
    if len(name_words) >= 2:
        keys.append(f"WW:{name_words[0]}_{name_words[1]}")

    # Medium-precision composite: addr_word + num
    for aw in addr_words[:2]:
        for nm in nums[:2]:
            keys.append(f"AN:{aw}_{nm}")

    # Standalone name words
    for nw in name_words[:3]:
        keys.append(f"W:{nw}")
...
# Compute key weights based on specificity
# Composite keys get high base weight, standalone words get IDF weight
N = float(us_count)
key_weights = {}
for k, id_list in idx.items():
    df = len(id_list)
    if df > 5000:
        continue # skip high-frequency noise
    idf = math.log((N + 1.0) / (df + 1.0))
    if k.startswith("WN:") or k.startswith("WW:"):
        key_weights[k] = idf * 2.5 # high precision
    elif k.startswith("AN:"):
...
q_time = time.time() - t0
print("\n=======================================================")
print(f"COMPOSITE KEY US S2 BLOCKING EVALUATION:")
